...
describe.only("Integration - route testing", async function () {
    const testUsers = {
        alice: {
            user: null,
            attributes: {
    ...
    for (let i = 0; i < 7; i++) {
        it("Should give 200 response if criterias are met and 400 if insufficent resources (eg. battery)", async () => {
            await postChaiRequest("hack/pettyCrime/", { userId: testUsers.alice.user._id.toString() }).then(result => {
                if (i === 6) {
    ...
    it(`POST CRIMES 30`, async () => {
        ...
        const crimeId = crimes[Math.floor(Math.random() * crimes.length)]._id.toString()
        await postChaiRequest("hack/crimes/", { userId: testUsers.bob.user._id.toString(), crimeId }).then(result => {
    ...
    it(`POST CRIMES 50`, async () => {
        ...
        const crimeId = crimes[Math.floor(Math.random() * crimes.length)]._id.toString()
        await postChaiRequest("hack/crimes/", { userId: testUsers.chuck.user._id.toString(), crimeId }).then(result => {
            console.log(result.body.finalResult, 'result crime')
            expect(result).to.have.status(200)
        });
    });
    ...
    it(`POST CRIMES 70`, async () => {
        ...
        const crimeId = crimes[Math.floor(Math.random() * crimes.length)]._id.toString()
        await postChaiRequest("hack/crimes/", { userId: testUsers.craig.user._id.toString(), crimeId }).then(result => {
            console.log(result.body.finalResult, 'result crime')
            expect(result).to.have.status(200)
        });
    });
    ...
    it(`POST CRIMES 90`, async () => {
        ...
        const crimeId = crimes[Math.floor(Math.random() * crimes.length)]._id.toString()
        await postChaiRequest("hack/crimes/", { userId: testUsers.eve.user._id.toString(), crimeId }).then(result => {
            console.log(result.body.finalResult, 'result crime')
            expect(result).to.have.status(200)
        });
    });
    ...
    it(`POST CRIMES 150`, async () => {
        ...
        const crimeId = crimes[Math.floor(Math.random() * crimes.length)]._id.toString()
        await postChaiRequest("hack/crimes/", { userId: testUsers.faythe.user._id.toString(), crimeId }).then(result => {
            console.log(result.body.finalResult, 'result crime')
            expect(result).to.have.status(200)
        });
    });
